...
const register = async (req, res,next) => {
  ...
  const hodExist = await User.findOne({ department , userType: "hod" });
    ...
    const nitrklEmailRegex = /@nitrkl\.ac\.in$/;

    if (!nitrklEmailRegex.test(email)) {
      return res.status(422).json({ error: "Kindly provide a email address associated with NIT Rourkela Institute" });
    }
    // Phone validation
    if (phone.length !== 10) {
    ...
    if (userFind) {
        ...
        if (setUserToken) {
          const mailOptions = {
            ...
            to:email,
            subject:"BookMyHall Reset Password",
    ...
    if (userFind) {
        ...
        if (setUserToken) {
          const mailOptions = {
            ...
            subject:"BookMyHall User Verification",
